		...
		$pcConfig = array(
			...
			'Invoice' => array(
				'Customer' => array(
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(0 => 'ID', 1 => 'Type', 2 => 'Number', 3 => 'Date', 5 => 'Customer', 6 => 'Phone', 7 => 'Email', 10 => 'Country', 11 => 'Payment Status', 12 => 'Due amount', 13 => 'Amount paid', 15 => 'Status', 16 => 'Tax', 17 => 'Total', 22 => 'Related'),
					'display-field-names' => array(0 => 'id', 1 => 'type', 2 => 'number', 3 => 'Date', 5 => 'Customer', 6 => 'Phone', 7 => 'Email', 10 => 'Country', 11 => 'PaymentStatus', 12 => 'AmountDUE', 13 => 'AmountPAID', 15 => 'Status', 16 => 'tax', 17 => 'Total', 22 => 'related'),
			...
			'Accounting' => array(
				'invoice' => array(
					...
					'child-primary-key' => 'id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Invoice Cash Flow <span class="hidden child-label-Accounting child-field-caption">(Invoice)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/Cashflow.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Invoice', 2 => 'Date', 3 => 'Description', 4 => 'Account plan', 5 => 'Master account', 6 => 'Account', 7 => 'Subaccount', 8 => 'Type', 9 => 'Amount', 10 => 'Balance', 11 => 'Status'),
					'display-field-names' => array(1 => 'invoice', 2 => 'date', 3 => 'description', 4 => 'account_plan', 5 => 'master_account', 6 => 'account', 7 => 'sub_account', 8 => 'type', 9 => 'amount', 10 => 'balance', 11 => 'status'),
					'sortable-fields' => array(0 => '`Accounting`.`id`', 1 => 2, 2 => '`Accounting`.`date`', 3 => 4, 4 => 5, 5 => 6, 6 => 7, 7 => 8, 8 => 9, 9 => '`Accounting`.`amount`', 10 => '`Accounting`.`balance`', 11 => 12),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-Accounting',
					'template-printable' => 'children-Accounting-printable',
					'query' => "SELECT `Accounting`.`id` as 'id', IF(    CHAR_LENGTH(`Invoice1`.`number`) || CHAR_LENGTH(`Customers1`.`Customer`), CONCAT_WS('',   `Invoice1`.`number`, ' - ', `Customers1`.`Customer`), '') as 'invoice', if(`Accounting`.`date`,date_format(`Accounting`.`date`,'%m/%d/%Y'),'') as 'date', `Accounting`.`description` as 'description', IF(    CHAR_LENGTH(`AccountPlan1`.`description`) || CHAR_LENGTH(`AccountPlan1`.`code`), CONCAT_WS('',   `AccountPlan1`.`description`, ' - ', `AccountPlan1`.`code`), '') as 'account_plan', IF(    CHAR_LENGTH(`MasterAccount1`.`masterAccount`) || CHAR_LENGTH(`MasterAccount1`.`code`), CONCAT_WS('',   `MasterAccount1`.`masterAccount`, ' - ', `MasterAccount1`.`code`), '') as 'master_account', IF(    CHAR_LENGTH(`Account1`.`Account`) || CHAR_LENGTH(`Account1`.`code`), CONCAT_WS('',   `Account1`.`Account`, ' - ', `Account1`.`code`), '') as 'account', IF(    CHAR_LENGTH(`SubAccount1`.`subAccount`) || CHAR_LENGTH(`SubAccount1`.`code`), CONCAT_WS('',   `SubAccount1`.`subAccount`, ' - ', `SubAccount1`.`code`), '') as 'sub_account', IF(    CHAR_LENGTH(`Type1`.`type`), CONCAT_WS('',   `Type1`.`type`), '') as 'type', FORMAT(`Accounting`.`amount`, 2) as 'amount', `Accounting`.`balance` as 'balance', `Accounting`.`status` as 'status' FROM `Accounting` LEFT JOIN `Invoice` as Invoice1 ON `Invoice1`.`id`=`Accounting`.`invoice` LEFT JOIN `Customers` as Customers1 ON `Customers1`.`id`=`Invoice1`.`Customer` LEFT JOIN `AccountPlan` as AccountPlan1 ON `AccountPlan1`.`id`=`Accounting`.`account_plan` LEFT JOIN `MasterAccount` as MasterAccount1 ON `MasterAccount1`.`id`=`AccountPlan1`.`master_account` LEFT JOIN `Account` as Account1 ON `Account1`.`id`=`AccountPlan1`.`account` LEFT JOIN `SubAccount` as SubAccount1 ON `SubAccount1`.`id`=`AccountPlan1`.`sub_account` LEFT JOIN `Type` as Type1 ON `Type1`.`id`=`AccountPlan1`.`type` "
				),
			),
			'AccountPlan' => array(
			),
