...
function Return_insert() {
	...
	$data['Refund'] = $_REQUEST['Refund'];
		if($data['Refund'] == empty_lookup_value) { $data['Refund'] = ''; }
	if($data['Status']== '') {
	...
	insert('Return', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) Return_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function Return_copy_children($destination_id, $source_id) {
	global $Translation;
...
function Return_update($selected_id) {
	...
	$data['Amount'] = makeSafe($_REQUEST['Amount']);
		if($data['Amount'] == empty_lookup_value) { $data['Amount'] = ''; }
	$data['Refund'] = makeSafe($_REQUEST['Refund']);
		if($data['Refund'] == empty_lookup_value) { $data['Refund'] = ''; }
	...
	sql('update `Return` set       `Date`=' . (($data['Date'] !== '' && $data['Date'] !== NULL) ? "'{$data['Date']}'" : 'NULL') . ', `Invoice`=' . (($data['Invoice'] !== '' && $data['Invoice'] !== NULL) ? "'{$data['Invoice']}'" : 'NULL') . ', `Warehouse`=' . (($data['Warehouse'] !== '' && $data['Warehouse'] !== NULL) ? "'{$data['Warehouse']}'" : 'NULL') . ', `Tracking`=' . (($data['Tracking'] !== '' && $data['Tracking'] !== NULL) ? "'{$data['Tracking']}'" : 'NULL') . ', `Shippingt`=' . (($data['Shippingt'] !== '' && $data['Shippingt'] !== NULL) ? "'{$data['Shippingt']}'" : 'NULL') . ', `Status`=' . (($data['Status'] !== '' && $data['Status'] !== NULL) ? "'{$data['Status']}'" : 'NULL') . ', `Amount`=' . (($data['Amount'] !== '' && $data['Amount'] !== NULL) ? "'{$data['Amount']}'" : 'NULL') . ', `Refund`=' . (($data['Refund'] !== '' && $data['Refund'] !== NULL) ? "'{$data['Refund']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!='') {
		echo $o['error'];
...
function Return_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if(is_file(dirname(__FILE__).'/hooks/Return.Shippingt.csv')) {
		$Shippingt_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/Return.Shippingt.csv')));
	...
	if($selected_id) {
		...
		$combo_Shippingt->SelectedText = ( $_REQUEST['FilterField'][1]=='6' && $_REQUEST['FilterOperator'][1]=='<=>' ? (get_magic_quotes_gpc() ? stripslashes($_REQUEST['FilterValue'][1]) : $_REQUEST['FilterValue'][1]) : "");
			...
			$j("#Warehouse-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_Warehouse__RAND__.value, t: 'Return', f: 'Warehouse' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { /* */ return { s: term, p: page, t: 'Return', f: 'Warehouse' }; },
			...
			$j("#Tracking-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_Tracking__RAND__.value, t: 'Return', f: 'Tracking' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { /* */ return { s: term, p: page, t: 'Return', f: 'Tracking' }; },
					results: function(resp, page) { /* */ return resp; }
				},
				escapeMarkup: function(str) { /* */ return str; }
			}).on('change', function(e) {
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		...
		$jsReadOnly .= "\tjQuery('#Refund').prop('disabled', true);\n";
	...
	$templateCode = str_replace('<%%UPLOADFILE(Refund)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		$templateCode = str_replace('<%%CHECKED(Refund)%%>', ($row['Refund'] ? "checked" : ""), $templateCode);
	}else{
		...
		$templateCode = str_replace('<%%CHECKED(Refund)%%>', '', $templateCode);
	}
	...
	$templateCode .= loadView('Return-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
