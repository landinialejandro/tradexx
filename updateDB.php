<?php
	...
	if($thisMD5==$prevMD5) {
		...
		setupTable('Accounting', "create table if not exists `Accounting` (   `id` INT unsigned not null auto_increment , primary key (`id`), `invoice` INT unsigned null , `date` DATE null , `description` TEXT null , `master_acount` INT unsigned null , `account` INT unsigned null , `sub_account` INT unsigned null , `type` INT unsigned null , `amount` DECIMAL(10,2) null , `balance` DECIMAL(10,2) null ) CHARSET utf8", $silent);
		setupIndexes('Accounting', array('invoice','master_acount','account','sub_account','type'));
