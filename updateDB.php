<?php
	...
	if($thisMD5==$prevMD5) {
		...
		setupTable('Invoice', "create table if not exists `Invoice` (   `id` INT unsigned not null auto_increment , primary key (`id`), `Date` DATE null , `Title` INT unsigned null , `Customer` INT unsigned null , `Phone` INT unsigned null , `Email` INT unsigned null , `Address` INT unsigned null , `City` INT unsigned null , `Country` INT unsigned null , `PaymentStatus` VARCHAR(40) null default 'UNPAID' , `AmountDUE` DECIMAL(10,2) null , `AmountPAID` DECIMAL(10,2) null , `Balance` DECIMAL(10,2) null , `Status` VARCHAR(40) null default 'OPEN' ) CHARSET utf8", $silent);
		setupIndexes('Invoice', array('Customer'));
		setupTable('Products', "create table if not exists `Products` (   `id` INT unsigned not null auto_increment , primary key (`id`), `invoice` INT unsigned null , `item` VARCHAR(40) null , `qty` VARCHAR(40) null , `total` VARCHAR(40) null , `uploads` VARCHAR(40) null ) CHARSET utf8", $silent);
