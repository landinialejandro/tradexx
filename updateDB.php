<?php
	...
	if($thisMD5==$prevMD5) {
		...
		setupTable('InvoiceDetails', "create table if not exists `InvoiceDetails` (   `id` INT unsigned not null auto_increment , primary key (`id`), `invoice` INT unsigned null , `order` INT null , `product` INT unsigned null , `qty` DECIMAL(10,2) null , `itemSale` INT unsigned null , `SubTotal` VARCHAR(40) null ) CHARSET utf8", $silent);
		...
		setupTable('Accounting', "create table if not exists `Accounting` (   `id` INT unsigned not null auto_increment , primary key (`id`), `invoice` INT unsigned null , `date` DATE null , `description` TEXT null , `account_plan` INT unsigned null , `master_account` INT unsigned null , `account` INT unsigned null , `sub_account` INT unsigned null , `type` INT unsigned null , `amount` DECIMAL(10,2) null , `balance` DECIMAL(10,2) null , `status` VARCHAR(40) null default 'OPEN' ) CHARSET utf8", $silent, array( "ALTER TABLE `Accounting` CHANGE `master_acount` `master_account` INT unsigned null ","ALTER TABLE Accounting ADD `field12` VARCHAR(40)","ALTER TABLE `Accounting` CHANGE `field12` `status` VARCHAR(40) null "," ALTER TABLE `Accounting` CHANGE `status` `status` VARCHAR(40) null default 'OPEN' "));
		setupIndexes('Accounting', array('invoice','account_plan'));
		setupTable('AccountPlan', "create table if not exists `AccountPlan` (   `id` INT unsigned not null auto_increment , primary key (`id`), `description` TEXT not null , `code` VARCHAR(40) not null , `master_account` INT unsigned not null , `account` INT unsigned not null , `sub_account` INT unsigned not null , `type` INT unsigned not null ) CHARSET utf8", $silent, array( " ALTER TABLE `AccountPlan` CHANGE `description` `description` TEXT not null "," ALTER TABLE `AccountPlan` CHANGE `code` `code` VARCHAR(40) not null "," ALTER TABLE `AccountPlan` CHANGE `master_account` `master_account` INT unsigned not null "," ALTER TABLE `AccountPlan` CHANGE `account` `account` INT unsigned not null "," ALTER TABLE `AccountPlan` CHANGE `sub_account` `sub_account` INT unsigned not null "," ALTER TABLE `AccountPlan` CHANGE `type` `type` INT unsigned not null "));
		...
		setupTable('Account', "create table if not exists `Account` (   `id` INT unsigned not null auto_increment , primary key (`id`), `Account` VARCHAR(40) null , `masterAccount` INT unsigned null , `code` VARCHAR(40) null ) CHARSET utf8", $silent);
		setupIndexes('Account', array('masterAccount'));
		setupTable('SubAccount', "create table if not exists `SubAccount` (   `id` INT unsigned not null auto_increment , primary key (`id`), `account` INT unsigned null , `subAccount` VARCHAR(40) null , `code` VARCHAR(40) null ) CHARSET utf8", $silent);
